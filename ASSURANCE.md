...
| Skips remain successful, and diagnostics preserve stdout/quiet contracts. | `copy_names_each_skipped_file_and_reason`, `copy_skip_lines_are_suppressed_under_quiet`, `copy_skip_line_matches_the_dry_run_wording`; output is checked separately from preserved destination contents. | Actual Linux/macOS/Windows CI; overwrite remedy text is Unix-specific. |
| Empty-selection hints describe skipped rules without reading their targets or promising a cure. | `ignored_invalid_syntax_symlink_hint_is_conditional`, `ignored_mixed_rule_symlinks_do_not_promise_success`, `ignored_rule_symlink_to_fifo_does_not_block_diagnosis`. | Both backends on Unix; selection and validation semantics unchanged. |
